...
const ProjectNav = ({
  ...
  return (
    <div className="flex flex-col items-end justify-center w-full h-full gap-3">
      {Array.from(Array(projectCount), (e, i) => {
        return (
          <button
            className={`h-8 w-2 overflow-hidden rounded-full transition-colors duration-1000 ${colors[curKey + 1][0]}`}
            key={i}
            onClick={() => scroll.scrollTo(scroll.offset(`#project-${i}`, '200px'), true)}
